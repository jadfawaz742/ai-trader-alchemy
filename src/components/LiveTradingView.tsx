...
export const LiveTradingView: React.FC<{ symbols?: string[] }> = ({ symbols: configSymbols }) => {
  ...
  const analyzeAndTrade = useCallback(async (currentMarketData: MarketFluctuation[]) => {
    try {
      ...
      console.log('🤖 Calling advanced-trading-bot with symbols:', symbols);
      ...
      if (data?.signals && data.signals.length > 0) {
        ...
        console.log('⚠️ No trading signals generated by bot');
        setBotStatus('idle');
      }
    } catch (error) {
    ...
    const fetchRealMarketData = async () => {
      try {
        ...
        if (newMarketData.length > 0) {
          ...
          console.log('✅ Market data loaded successfully, triggering bot analysis...');
